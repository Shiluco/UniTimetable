...
func (mq *MajorQuery) Order(o ...major.OrderOption) *MajorQuery {
...
func (mq *MajorQuery) QueryUsers() *UserQuery {
...
func (mq *MajorQuery) Clone() *MajorQuery {
	...
	return &MajorQuery{
		config:         mq.config,
		ctx:            mq.ctx.Clone(),
		order:          append([]major.OrderOption{}, mq.order...),
...
func (mq *MajorQuery) WithUsers(opts ...func(*UserQuery)) *MajorQuery {
	query := (&UserClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//
//	client.Major.Query().
//		GroupBy(major.FieldDepartmentID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Major.Query().
//		Select(major.FieldDepartmentID).
//		Scan(ctx, &v)
...
func (mq *MajorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Major, error) {
	...
	if query := mq.withUsers; query != nil {
		if err := mq.loadUsers(ctx, query, nodes,
			func(n *Major) { n.Edges.Users = []*User{} },
			func(n *Major, e *User) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MajorQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Major, init func(*Major), assign func(*Major, *User)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(user.FieldMajorID)
	}
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(major.UsersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "major_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (mq *MajorQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := mq.ctx.Fields; len(fields) > 0 {
		...
		if mq.withDepartment != nil {
			_spec.Node.AddColumnOnce(major.FieldDepartmentID)
		}
	}
