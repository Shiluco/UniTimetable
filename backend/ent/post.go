...
type Post struct {
	...
	// ID of the ent.
	ID int `json:"post_id"`
	// ParentPostID holds the value of the "parent_post_id" field.
	...
	for i := range columns {
		...
		case post.FieldParentPostID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field parent_post_id", values[i])
			} else if value.Valid {
				po.ParentPostID = new(int)
				*po.ParentPostID = int(value.Int64)
			}
...
func (po *Post) String() string {
	...
	if v := po.ParentPostID; v != nil {
		builder.WriteString("parent_post_id=")
