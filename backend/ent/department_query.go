...
func (dq *DepartmentQuery) QueryMajors() *MajorQuery {
...
func (dq *DepartmentQuery) Clone() *DepartmentQuery {
	...
	return &DepartmentQuery{
		...
		withMajors: dq.withMajors.Clone(),
...
func (dq *DepartmentQuery) WithMajors(opts ...func(*MajorQuery)) *DepartmentQuery {
	query := (&MajorClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	...
	if query := dq.withMajors; query != nil {
		if err := dq.loadMajors(ctx, query, nodes,
			func(n *Department) { n.Edges.Majors = []*Major{} },
			func(n *Department, e *Major) { n.Edges.Majors = append(n.Edges.Majors, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DepartmentQuery) loadMajors(ctx context.Context, query *MajorQuery, nodes []*Department, init func(*Department), assign func(*Department, *Major)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Department)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(major.FieldDepartmentID)
	}
	query.Where(predicate.Major(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(department.MajorsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
